...
- 2026-08-19: Homepage redesigned to user's other-project layout: navy centered hero (kicker, big title, subcopy, "Request a service" CTA), navy stats bar (12+/6/24h/2), light "Who we are" two-column, light "Core capabilities" numbered rows (IT-01/IT-02/DM-01/DM-02 per-discipline numbering, View IT/Marketing page buttons), light "What clients notice first" outcomes, navy closing CTA (Request work / WhatsApp / Email). Hero/overview/capabilities/request copy still driven by Site editor content; capability rows driven by services API
- 2026-08-19: put_content now merges instead of replacing (bug fix — partial PUT had wiped fields); content reseeded
- 2026-08-19: Hero background = self-hosted looping video (webm VP9 + mp4 H.264 fallback via object storage /api/files/onusminds/news/ab32656d...webm) + NetworkCanvas particles overlay; select dropdown options forced dark bg (white-on-white bug on some OS); Timeline & Milestones and Our Presence removed from nav/footer/routes; service page Outcomes stat box removed; homepage stats + footer blurb + LinkedIn URL now editable in Site editor (stat_1..4 value/label, footer_blurb, linkedin_url keys)
- 2026-08-19: Activity log (owner-only sidebar view): records every sign-in, site content publish (with changed field names), services update, article create/edit/delete, file upload, request status change/delete, team account create/remove — user name, email, action, detail, timestamp. Third owner account added: sunnyjarial@gmail.com
## Backlog
